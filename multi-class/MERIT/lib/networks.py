...
from lib.decoders import CASCADE_Add, CASCADE_Cat, CASCADE_Add_dual
...
class MERIT_Parallel_dual(nn.Module):
    def __init__(self, n_class=1, img_size_s1=(256,256), img_size_s2=(224,224), model_scale='small', decoder_aggregation='additive', interpolation='bilinear'):
        super(MERIT_Parallel_dual, self).__init__()
        ...
        # shared decoder initialization
        if(self.decoder_aggregation=='additive'):
            self.decoder = CASCADE_Add_dual(channels=self.channels, num_class=self.n_class)
        elif(self.decoder_aggregation=='concatenation'):
            self.decoder = CASCADE_Cat(channels=self.channels)
        else:
            sys.exit("'"+self.decoder_aggregation+"' is not a valid decoder aggregation! Currently supported aggregations are 'additive' and 'concatenation'.")
    ...
    def forward(self, x):
        ...
        # decoder
        p11, p12, p13, p14, p11_bg, p12_bg, p13_bg, p14_bg, _ = self.decoder(f1[3], [f1[2], f1[1], f1[0]])
        
        p21, p22, p23, p24, p21_bg, p22_bg, p23_bg, p24_bg, _ = self.decoder(f2[3], [f2[2], f2[1], f2[0]])
        ...
        p11 = F.interpolate(p11, scale_factor=32, mode=self.interpolation)
        p11_bg = F.interpolate(p11_bg, scale_factor=32, mode=self.interpolation)
        p12 = F.interpolate(p12, scale_factor=16, mode=self.interpolation)
        p12_bg = F.interpolate(p12_bg, scale_factor=16, mode=self.interpolation)
        p13 = F.interpolate(p13, scale_factor=8, mode=self.interpolation)
        p13_bg = F.interpolate(p13_bg, scale_factor=8, mode=self.interpolation)
        p14 = F.interpolate(p14, scale_factor=4, mode=self.interpolation)
        p14_bg = F.interpolate(p14_bg, scale_factor=4, mode=self.interpolation)
        
        p21 = F.interpolate(p21, size=(p11.shape[-2:]), mode=self.interpolation)
        p21_bg= F.interpolate(p21_bg, size=(p11.shape[-2:]), mode=self.interpolation)
        p22 = F.interpolate(p22, size=(p12.shape[-2:]), mode=self.interpolation)
        p22_bg= F.interpolate(p22_bg, size=(p12.shape[-2:]), mode=self.interpolation)
        p23 = F.interpolate(p23, size=(p13.shape[-2:]), mode=self.interpolation)
        p23_bg= F.interpolate(p23_bg, size=(p13.shape[-2:]), mode=self.interpolation)
        p24 = F.interpolate(p24, size=(p14.shape[-2:]), mode=self.interpolation)
        p24_bg= F.interpolate(p24_bg, size=(p14.shape[-2:]), mode=self.interpolation)
        ...
        return p1, p2, p3, p4, p1_bg, p2_bg, p3_bg, p4_bg
...
class MERIT_Cascaded(nn.Module):
    ...
    def forward(self, x):
        ...
        # print([f1[3].shape,f1[2].shape,f1[1].shape,f1[0].shape]) # [12, 768, 8, 8] [12, 384, 16, 16] [12, 192, 32, 32] [12, 96, 64, 64] 
        
        # decoder
        x11_o, x12_o, x13_o, x14_o = self.decoder(f1[3], [f1[2], f1[1], f1[0]])

        # prediction heads  
        p11 = self.out_head1(x11_o) # [12, 4, 8, 8]
        p12 = self.out_head2(x12_o) # [12, 4, 16, 16]
        p13 = self.out_head3(x13_o) # [12, 4, 32, 32]
        p14 = self.out_head4(x14_o) # [12, 4, 64, 64]
        
        # calculate feedback from 1st decoder
        p14_in = self.out_head4_in(x14_o)
        p14_in = self.sigmoid(p14_in) # [12, 1, 256, 256]
        

        p11 = F.interpolate(p11, scale_factor=32, mode=self.interpolation) # [12, 4, 256, 256]
        p12 = F.interpolate(p12, scale_factor=16, mode=self.interpolation) # [12, 4, 256, 256]
        p13 = F.interpolate(p13, scale_factor=8, mode=self.interpolation) # [12, 4, 256, 256]
        p14 = F.interpolate(p14, scale_factor=4, mode=self.interpolation) # [12, 4, 256, 256]
        ...
        x_in = x * p14_in # [12, 3, 256, 256]
        
        if(x.shape[2]%14!=0):
            f2 = self.backbone2(F.interpolate(x_in, size=self.img_size_s2, mode=self.interpolation)) # [12, 96, 56, 56] [12, 192, 28, 28] [12, 384, 14, 14] [12, 768, 7, 7]
        else:
            f2 = self.backbone1(F.interpolate(x_in, size=self.img_size_s2, mode=self.interpolation))
            
        skip1_0 = F.interpolate(f1[0], size=(f2[0].shape[-2:]), mode=self.interpolation) # [12, 96, 64, 64]--->[12, 96, 56, 56]
        skip1_1 = F.interpolate(f1[1], size=(f2[1].shape[-2:]), mode=self.interpolation) # [12, 192, 32, 32]--->[12, 192, 28, 28]
        skip1_2 = F.interpolate(f1[2], size=(f2[2].shape[-2:]), mode=self.interpolation) # [12, 384, 16, 16]--->[12, 384, 14, 14]
        skip1_3 = F.interpolate(f1[3], size=(f2[3].shape[-2:]), mode=self.interpolation) # [12, 768, 8, 8]--->[12, 768, 7, 7]
...
class MERIT_Cascaded_dual(nn.Module):
    def __init__(self, n_class=1, img_size_s1=(256,256), img_size_s2=(224,224), model_scale='small', decoder_aggregation='additive', interpolation='bilinear'):
        super(MERIT_Cascaded_dual, self).__init__()
    ...
    def forward(self, x):
        ...
        # decoder
        p11, p12, p13, p14, p11_bg, p12_bg, p13_bg, p14_bg, x14_o = self.decoder(f1[3], [f1[2], f1[1], f1[0]])

        # 不需要分类头，因为双监督在decoder中已经实现了分类卷积头
        # p11 = x11_o
        # p12 = x12_o
        # p13 = x13_o
        # p14 = x14_o
        ...
        p21, p22, p23, p24, p21_bg, p22_bg, p23_bg, p24_bg, _  = self.decoder(f2[3]+skip1_3, [f2[2]+skip1_2, f2[1]+skip1_1, f2[0]+skip1_0])
