...
def convert_labels_to_one_hot_masks(batch_labels, num_classes):
    ...
    # 创建一个形状为 [bs, num_classes, H, W] 的零张量
    one_hot_masks = torch.zeros((bs, num_classes, H, W), dtype=torch.uint8, device=batch_labels.device)

    # 使用scatter方法进行one-hot编码
    # 将batch_labels中每个像素的值作为索引，将one_hot_masks的相应位置置为1
...
def trainer_synapse(args, model, snapshot_path):
    ...
    logging.info("########## Using Dual Supervision ##########" if args.dual else "########## Using Single Supervision ##########")
    logging.info("########## Using MERIT_Cascaded ##########" if args.merit_type == 'MERIT_Cascaded' else 
                 "########## Using MERIT_Parallel ##########" if args.merit_type == 'MERIT_Parallel' else 
                 "ERROR")
    logging.info("##########Disable test_save##########" if args.test_save_dir is None else "##########Enable test_save##########")
