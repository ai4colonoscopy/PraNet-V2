...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        q = q.view(x.shape[0], x.shape[1], x.shape[2] * x.shape[3]) # (batch_size, channels, height, width) 变换为 (batch_size, channels, height * width)
...
class CAM(nn.Module):
    def __init__(self, args, **kwargs):
        ...
        # RA addition
        self.channels = kwargs.get('channels', False)
        self.n_class = kwargs.get('n_class', 0)
        if self.channels and self.n_class!=0:
            self.out_head1_fg = nn.Conv2d(self.channels[0], self.n_class, 1)
            self.out_head2_fg = nn.Conv2d(self.channels[1], self.n_class, 1)
            self.out_head3_fg = nn.Conv2d(self.channels[2], self.n_class, 1)
            self.out_head4_fg = nn.Conv2d(self.channels[3], self.n_class, 1)

            self.out_head1_bg = nn.Conv2d(self.channels[0], self.n_class, 1)
            self.out_head2_bg = nn.Conv2d(self.channels[1], self.n_class, 1)
            self.out_head3_bg = nn.Conv2d(self.channels[2], self.n_class, 1)
            self.out_head4_bg = nn.Conv2d(self.channels[3], self.n_class, 1)
