...
from lib.decoders import EMCAD, EMCAD_dual


class EMCADNet(nn.Module):
    def __init__(self, num_classes=1, kernel_sizes=[1,3,5], expansion_factor=2, dw_parallel=True, add=True, lgag_ks=3, activation='relu', encoder='pvt_v2_b2', pretrain=True,**kwargs):
        ...
        if pretrain==True and 'pvt_v2' in encoder:
            # path='/defaultShare/archive/zhuzixuan/hubocheng/EMCAD/pretrained_pth/pvt/pvt_v2_b2.pth'
        ...
        if self.dual:
            self.decoder = EMCAD_dual(channels=channels, kernel_sizes=kernel_sizes, expansion_factor=expansion_factor, dw_parallel=dw_parallel, add=add, lgag_ks=lgag_ks, activation=activation,num_class=num_classes)
            print('dual supervision is applied in EMCADNet.') 
        else:
            self.decoder = EMCAD(channels=channels, kernel_sizes=kernel_sizes, expansion_factor=expansion_factor, dw_parallel=dw_parallel, add=add, lgag_ks=lgag_ks, activation=activation)
            print('single supervision is applied in EMCADNet.')
    ...
    def forward(self, x, mode='test'):
        ...
        if self.dual:
            p11, p12, p13, p14, p11_bg, p12_bg, p13_bg, p14_bg=dec_outs
            ...
            return [p11, p12, p13, p14, p11_bg, p12_bg, p13_bg, p14_bg]
        else:
