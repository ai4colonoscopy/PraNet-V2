...
class EMCAD_dual(nn.Module):
    def __init__(self, channels=[512,320,128,64], kernel_sizes=[1,3,5], expansion_factor=6, dw_parallel=True, add=True, lgag_ks=3, activation='relu6',num_class=None):
        super(EMCAD_dual,self).__init__()
        ...
        # dual supervision additional layers
        self.ConvBlock4_fg=BasicConv2d(channels[0], num_class, kernel_size=1) # DSRA newly added, for foreground
        self.ConvBlock4_bg=BasicConv2d(channels[0], num_class, kernel_size=1) # DSRA newly added, for background
        
        self.ConvBlock3_fg=BasicConv2d(channels[1], num_class, kernel_size=3, padding=1) # DSRA newly added, for foreground
        self.ConvBlock3_bg=BasicConv2d(channels[1], num_class, kernel_size=3, padding=1) # DSRA newly added, for background
        
        self.ConvBlock2_fg=BasicConv2d(channels[2], num_class, kernel_size=3, padding=1) # DSRA newly added, for foreground
        self.ConvBlock2_bg=BasicConv2d(channels[2], num_class, kernel_size=3, padding=1) # DSRA newly added, for background
       
        self.ConvBlock1_fg=BasicConv2d(channels[3], num_class, kernel_size=3, padding=1) # DSRA newly added, for foreground
        self.ConvBlock1_bg=BasicConv2d(channels[3], num_class, kernel_size=3, padding=1) # DSRA newly added, for background
    ...
    def forward(self, x, skips):
        ...
        d4 = self.cab4(x)*x
        d4 = self.sab(d4)*d4 
        d4 = self.mscb4(d4) # [12, 512, 8, 8]
        
        d4_fg = self.ConvBlock4_fg(d4) # [12, num_class, 8, 8]
        d4_bg = self.ConvBlock4_bg(d4) # [12, num_class, 8, 8]
        
        # EUCB3
        d3 = self.eucb3(d4) # [12, 320, 16, 16]
        
        d3_up_fg=F.interpolate(d4_fg, size=d3.size()[2:], mode='bilinear') # [12, num_class, 16, 16]
        d3_up_bg=F.interpolate(d4_bg, size=d3.size()[2:], mode='bilinear') # [12, num_class, 16, 16]
                 
        # LGAG3
        x3 = self.lgag3(g=d3, x=skips[0]) # [12, 320, 16, 16]
        ...
        d3 = self.cab3(d3)*d3
        d3 = self.sab(d3)*d3  
        d3 = self.mscb3(d3) # [12, 320, 16, 16]
        
        d3_fg = self.ConvBlock3_fg(d3) # [12, num_class, 16, 16]
        d3_bg = self.ConvBlock3_bg(d3) # [12, num_class, 16, 16]
        
        d3_fg = d3_fg+d3_fg.mul(nn.functional.softmax(d3_up_fg-d3_up_bg, dim=1))

        
        # EUCB2
        d2 = self.eucb2(d3) # [12, 128, 32, 32]
        
        d2_up_fg=F.interpolate(d3_fg, size=d2.size()[2:], mode='bilinear') # [12, num_class, 32, 32]
        d2_up_bg=F.interpolate(d3_bg, size=d2.size()[2:], mode='bilinear') # [12, num_class, 32, 32]
        ...
        d2 = self.cab2(d2)*d2
        d2 = self.sab(d2)*d2
        d2 = self.mscb2(d2) # [12, 128, 32, 32]
        
        d2_fg = self.ConvBlock2_fg(d2) # [12, num_class, 32, 32]
        d2_bg = self.ConvBlock2_bg(d2) # [12, num_class, 32, 32]
        
        d2_fg = d2_fg+d2_fg.mul(nn.functional.softmax(d2_up_fg-d2_up_bg, dim=1))

        
        # EUCB1
        d1 = self.eucb1(d2) # [12, 64, 64, 64]
        
        d1_up_fg=F.interpolate(d2_fg, size=d1.size()[2:], mode='bilinear') # [12, num_class, 64, 64]
        d1_up_bg=F.interpolate(d2_bg, size=d1.size()[2:], mode='bilinear') # [12, num_class, 64, 64]
        ...
        d1 = self.cab1(d1)*d1
        d1 = self.sab(d1)*d1
        d1 = self.mscb1(d1) # [12, 64, 64, 64]
        
        d1_fg = self.ConvBlock1_fg(d1) # [12, num_class, 64, 64]
        d1_bg = self.ConvBlock1_bg(d1) # [12, num_class, 64, 64]
        
        d1_fg = d1_fg+d1_fg.mul(nn.functional.softmax(d1_up_fg-d1_up_bg, dim=1))

        
        return [d4_fg, d3_fg, d2_fg, d1_fg, d4_bg, d3_bg, d2_bg, d1_bg]
