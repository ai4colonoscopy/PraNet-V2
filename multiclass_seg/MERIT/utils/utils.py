...
def test_single_volume(image, label, net, classes, patch_size=[256, 256], test_save_path=None, case=None, z_spacing=1, class_names=None, use_dual=False):
        ...
        for ind in range(image.shape[0]):
            ...
            with torch.no_grad():
                ...
                if test_save_path is not None:
                    ...
                    fig_gt = overlay_masks(image[ind, :, :], masks, labels=mask_labels, colors=cmap, mask_alpha=0.5,return_pil_image=True)
                    fig_pred = overlay_masks(image[ind, :, :], preds_o, labels=mask_labels, colors=cmap, mask_alpha=0.5,return_pil_image=True)
                    # # Do with that image whatever you want to do.
                    # total_width = fig_gt.width + fig_pred.width
                    # max_height = max(fig_gt.height, fig_pred.height)
                    # # 创建一个新的图像用于拼接
                    # new_img = Image.new('RGB', (total_width+10, max_height))
                    # new_img.paste(fig_gt, (0, 0))  # 将第一个图像放在左边
                    # new_img.paste(fig_pred, (fig_gt.width+10, 0))  # 将第二个图像放在右边
                    # new_img.save(test_save_path + '/' + case + '_' + str(ind) + '_combined.png',bbox_inches="tight", dpi=300)
...
def val_single_volume(image, label, net, classes, patch_size=[256, 256], test_save_path=None, case=None, z_spacing=1,use_dual=False):
    image, label = image.squeeze(0).cpu().detach().numpy(), label.squeeze(0).cpu().detach().numpy()
