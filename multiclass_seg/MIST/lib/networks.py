...
class MIST_CAM(nn.Module):
    def __init__(self, n_class=1, img_size_s1=(256, 256), img_size_s2=(224, 224), model_scale='small',
                 decoder_aggregation='additive', interpolation='bilinear', **kwargs):
        ...
        if self.dual:
            print('Dual supervision is applied in MIST_CAM.') 
            self.decoder=CAM("SSS", channels=self.channels,n_class=self.n_class)
        else:
            print('Single supervision is applied in MIST_CAM.')
    ...
    def forward(self, x):
        ...
        if self.dual:
            p11, p12, p13, p14, p11_bg, p12_bg, p13_bg, p14_bg=self.decoder(f1[0], f1[1], f1[2], f1[3])
